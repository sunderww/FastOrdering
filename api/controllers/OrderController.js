...
module.exports = {
 ...
 create:function(req, res) {
     var json = new Object();
      json.numTable = req.param('numTable');
      json.numPA = req.param('dinerNumber');
      json.globalComment = req.param('comments');
      if (req.param('numOrder'))
        json.numOrder = req.param('numOrder');
      json.order = new Array();
  OrderServices.createOrder(req.session.user,json, function(ret){
    res.json({ret:ret})
  });
 },
...
delete: function(req, res) {
 Order.destroy({id:req.param('id')}).exec(function(err, doc) {
   return res.ok("ok");
 });
...
ready: function(req, res) {
  ...
  if (req.param("id")) {
    ...
    .then(function(ordered){
      var user = User.findOne({id:ordered.order.waiter_id}).then(function(user) {return user});
      ...
      if (current_status != "toDeliver") {
        ...
        sails.sockets.emit(socket_id.socket_id, 'notifications', data);
      }
      else {
        var msg = "Le plat " + dish + " n'est plus prêt!";
        msg = socket_id.email == "xavier@fastordering.fr" ? msg.latinise() : msg;
        var data = {date: moment().format("DD/MM/YY"),hour: moment().format("HH:mm"),msg: msg, numTable:numTable}
        sails.sockets.emit(socket_id.socket_id, 'notifications', data);
      }
