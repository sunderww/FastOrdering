...
module.exports = function(req, res, next) {
  ...
  if (req.session.user) {
  	Table.findOne(req.param('id')).done(function (err, table) {
  		if (err) return res.serverError(err);
  		if (!table) return res.notFound();

		Restaurant.findOne({plan_id: table.plan_id}).done(function (err, restaurant) {
			if (err) return res.serverError(err);
