/**
 * orderedDishBelongsToUserOrAdmin
 *
 * @module      :: Policy
...
module.exports = function(req, res, next) {
  ...
  if (req.session.user) {
  	OrderedDish.findOne(req.param('id')).done(function (err, orderedDish) {
  		if (err) return res.serverError(err);
  		if (!orderedDish) return res.notFound();

		Dish.findOne(orderedDish.order_id).done(function (err, dish) {
	  		if (err) return res.serverError(err);
